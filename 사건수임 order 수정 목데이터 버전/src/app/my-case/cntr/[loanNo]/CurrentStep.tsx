...
const DDay = ({
  ...
  const { seller, buyer, repayList, adminReqStatCd } = usePayInfoData({ loanNo });
  ...
  const hrefExtractor = () => {
    ...
    if(
      props.lndHndgSlfDsc === "1" &&
      (statCd === "10" || statCd === "11") &&
      ((adminReqStatCd === "00" || adminReqStatCd === "01" || adminReqStatCd === "02"))) {
      return "/my-case/pay-request/loan-info";
    }

    // 대출금 처리 > 대출 취급 주체 코드: 은행지점(1) + 상태 코드: 대출 실행(10) 또는 인증 확인 중(11) + 서류 제출 프로세스 미진행 초기 상태
    if (props.lndHndgSlfDsc === "1" && (statCd === "10" || statCd === "11") && adminReqStatCd === "")
